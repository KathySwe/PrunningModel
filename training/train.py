...
if __name__ == '__main__':
    ...
    lrate = tf.keras.callbacks.LearningRateScheduler(_step_decay)
    checkpoint = tf.keras.callbacks.ModelCheckpoint(weights_best_file, monitor='loss',
                                 verbose=0, save_best_only=False,
                                 save_weights_only=True, mode='min', period=1)
    csv_logger =tf.keras.callbacks.CSVLogger(training_log, append=True)
    tb = tf.keras.callbacks.TensorBoard(log_dir=logs_dir, histogram_freq=0, write_graph=True,
                     write_images=False)
